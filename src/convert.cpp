 ...
 */
void MainWindow::slot_showConvertResult(int returnCode, std::string inputFile)
{
    ui->progressBar->setValue(ui->progressBar->value() + 1);    // 更新进度条
    ui->textBrowserResult->append(tr("转换结果："));
    if (returnCode == 0) {
        ui->textBrowserResult->append(QString::fromStdString("✅ " + inputFile + "\n"));
    }
    else {
        ui->textBrowserResult->append(QString::fromStdString("❌ " + inputFile + "\n"));
    }
 ...
 */
void Convert::handleConvert(MainWindow* instance, QString inputFileRaw)
{
    ...
    // 输出文件的绝对路径（转码前）
    QString outputFileRaw = QDir(QString::fromStdString(instance->outputDirectory)).filePath(QString::fromStdString(
                                outputFileName));
    // 输出文件的绝对路径（转码后）
